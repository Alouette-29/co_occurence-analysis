...
def draw_heatmap(co_matrix: np.ndarray, freq_matrix: np.ndarray, people: list[str], filename: str):
    ...
    ax1.set_xticklabels(people, rotation = 45)
    ax1.set_title("co_matrix")
    for i in range(len(co_matrix)):
        for j in range(len(co_matrix[i])):
            ax1.text(j, i, int(co_matrix[i][j]),
                    ha="center", va="center", color="w", fontsize=12, fontname='Times New Roman')
